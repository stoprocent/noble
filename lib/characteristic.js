...
class Characteristic extends EventEmitter {
  ...
  async broadcastAsync (broadcast) {
    return this._noble._withDisconnectHandler(this._peripheralId, () => {
      return new Promise((resolve, reject) => {
        this.broadcast(broadcast, error => error ? reject(error) : resolve());
      });
    });
  }
  ...
  notify (notify, callback) {
    if (callback) {
      this.once('notify', error => callback(error));
    }
  ...
  async notifyAsync (notify) {
    return this._noble._withDisconnectHandler(this._peripheralId, () => {
      return new Promise((resolve, reject) => {
        this.notify(notify, error => error ? reject(error) : resolve());
      });
    });
  }
  ...
  async subscribeAsync () {
    return this._noble._withDisconnectHandler(this._peripheralId, () => {
      return new Promise((resolve, reject) => {
        this.subscribe(error => error ? reject(error) : resolve());
      });
    });
  }
