...
class Noble extends EventEmitter {
  ...
  _discoveredPeripherals = new Set();
  _peripherals = new Map();
  _services = {};
  _characteristics = {};
  _descriptors = {};
  _initialized = false;
  ...
  startScanning(serviceUuids, allowDuplicates, callback) {
    ...
    const scan = (state) => {
      ...
      if (state !== 'poweredOn') {
        ...
        console.log('_bindings.startScanning', serviceUuids, allowDuplicates);
  ...
  async connectAsync(peripheralUuid, parameters) {
    return new Promise((resolve, reject) => {
      this.connect(peripheralUuid, parameters, error => error ? reject(error) : resolve());
    });
  };
  ...
  _onRead(peripheralUuid, serviceUuid, characteristicUuid, data, isNotification, error) {
    ...
    if (characteristic) {
      ...
      characteristic.emit('read', data, isNotification, error); // for backwards compatbility
    } else {
