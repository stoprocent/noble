...
var HciSerialParser = require('./hci-serial-parser');
...
var EVT_COMPLETED_PACKETS = 0x13;
...
var OCF_LE_READ_BUFFER_SIZE = 0x02;

var READ_BUFFER_SIZE_CMD = OCF_READ_BUFFER_SIZE | OGF_INFO_PARAM << 10;
var LE_READ_BUFFER_SIZE_CMD = OCF_LE_READ_BUFFER_SIZE | OGF_LE_CTL << 1
...
Hci.prototype.init = function() {
  ...
  this._socket = new SerialPort(process.env.NOBLE_HCI_UART_PORT || undefined, {
    baudRate: process.env.NOBLE_HCI_UART_BAUD_RATE || 115200,
    autoOpen: false,
    flowControl: true
  });
  ...
  this._socketQueue = async.queue(function(task, callback) {
    this._socket.write(task.buffer, (a, b) => callback());
  }.bind(this));
...
Hci.prototype.writeAclDataPkt = function(handle, cid, data) {
  ...
  this._socketQueue.push({buffer: pkt });
};

Hci.prototype.onSocketData = function(data) {
  if (!this._isDevUp) return;
  ...
  if (HCI_EVENT_PKT === eventType) {
    ...
    debug('\tsub event type = ' + subEventType + ", hex: 0x" + data.subarray(1,2).toString('hex'));
    ...
    } else if (subEventType === EVT_CMD_COMPLETE && data.length > 2) {
    ...
    } else if (subEventType === EVT_COMPLETED_PACKETS) {
      this._aclPacketsCount = Math.max(0, this._aclPacketsCount - 1);
...
Hci.prototype.processCmdCompleteEvent = function(cmd, status, result) {
  if (cmd === RESET_CMD) {
    ...
    var aclDataPacketLength = result.readUInt16LE(0);
    var syncDataPacketLength = result.readUInt8(2);
    var totalNumAclDataPackets = result.readUInt16LE(3);
    var totalNumSyncDataPackets = result.readUInt16LE(5);
    console.log("LE_READ_BUFFER_SIZE_CMD", "aclDataPacketLength", aclDataPacketLength);
    console.log("LE_READ_BUFFER_SIZE_CMD", "syncDataPacketLength", syncDataPacketLength);
    console.log("LE_READ_BUFFER_SIZE_CMD", "totalNumAclDataPackets", totalNumAclDataPackets);
    console.log("LE_READ_BUFFER_SIZE_CMD", "totalNumSyncDataPackets", totalNumSyncDataPackets);
    // if (MAX_HCI_ACL_DATA_PACKETS == 0) {
    //   MAX_HCI_ACL_DATA_PACKETS = totalNumAclDataPackets;
    // }
  } else if (cmd === LE_READ_BUFFER_SIZE_CMD) {
    var leDataPacketLength = result.readUInt16LE(0);
    var totalNumLEDataPackets = result.readUInt8(2);
    console.log("LE_READ_BUFFER_SIZE_CMD", "leDataPacketLength", leDataPacketLength);
    console.log("LE_READ_BUFFER_SIZE_CMD", "totalNumLEDataPackets", totalNumLEDataPackets);
    // MAX_HCI_ACL_DATA_PACKETS = totalNumLEDataPackets;
